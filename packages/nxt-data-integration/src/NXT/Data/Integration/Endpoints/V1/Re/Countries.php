...
class Countries extends BaseEndpoint
{
    ...
    public function getById(array $params): Country
    {
        ...
        return new Country($this->send("get", array_filter($params, fn($key) => in_array($key, ['id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY)));
    }
    ...
    public function list_(array $params): CountryCollection
    {
        ...
        return new CountryCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['limit','offset']), ARRAY_FILTER_USE_KEY)));
    }
