...
class Nameformatconfigurations extends BaseEndpoint
{
    ...
    public function getById(array $params): NameFormatConfiguration
    {
        ...
        return new NameFormatConfiguration($this->send("get", array_filter($params, fn($key) => in_array($key, ['id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY)));
    }
    ...
    {
        return new NameFormatConfigurationCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY)));
    }
