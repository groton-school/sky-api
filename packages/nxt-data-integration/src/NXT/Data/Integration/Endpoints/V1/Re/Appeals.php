...
class Appeals extends BaseEndpoint
{
    ...
    public function getById(array $params): Appeal
    {
        ...
        return new Appeal($this->send("get", array_filter($params, fn($key) => in_array($key, ['id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY)));
    }
    ...
    public function list_(array $params): AppealCollection
    {
        ...
        return new AppealCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['appeal_id','description','include_inactive','limit','offset']), ARRAY_FILTER_USE_KEY)));
    }
