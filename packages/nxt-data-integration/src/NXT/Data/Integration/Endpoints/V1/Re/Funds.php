...
class Funds extends BaseEndpoint
{
    ...
    public function getById(array $params): Fund
    {
        ...
        return new Fund($this->send("get", array_filter($params, fn($key) => in_array($key, ['id']), ARRAY_FILTER_USE_KEY), []));
    }
    ...
    public function list_(array $params): FundCollection
    {
        ...
        return new FundCollection($this->send("get", [], array_filter($params, fn($key) => in_array($key, ['fund_id','description','include_inactive','limit','offset']), ARRAY_FILTER_USE_KEY)));
    }
