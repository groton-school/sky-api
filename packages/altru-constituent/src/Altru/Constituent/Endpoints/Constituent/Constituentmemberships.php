...
class Constituentmemberships extends BaseEndpoint
{
    ...
    public function searchByConstituentId(array $params): ConstituentMembershipsListCollection
    {
        ...
        return new ConstituentMembershipsListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
