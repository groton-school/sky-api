...
class Relationshipjobsinfo extends BaseEndpoint
{
    ...
    /**
     * This operation returns a list of all employment history for a
     * constituent.
     *
     * @param array{constituent_id: string, include_inactive: bool, limit:
     *   int, session_key: string, infinity_session: string,
    ...
    public function searchByConstituentId(array $params): RelationshipJobInfoListCollection
    {
        ...
        return new RelationshipJobInfoListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['include_inactive','limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
