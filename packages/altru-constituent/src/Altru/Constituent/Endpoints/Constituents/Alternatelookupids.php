...
use Blackbaud\SKY\Altru\Constituent\Components\ConstituentAlternateLookupIdListCollection;
...
class Alternatelookupids extends BaseEndpoint
{
     ...
     */
    protected string $url = "https://api.sky.blackbaud.com/alt-conmg/constituents/{constituent_id}/alternatelookupids";
     ...
     *   rows.
     *
     * @return \Blackbaud\SKY\Altru\Constituent\Components\ConstituentAlternateLookupIdListCollection
     ...
     */
    public function searchByConstituentId(array $params): ConstituentAlternateLookupIdListCollection
    {
        ...
        return new ConstituentAlternateLookupIdListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
