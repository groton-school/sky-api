...
use Blackbaud\SKY\Altru\Constituent\Components\ConstituentAddressListCollection;
...
class Addresses extends BaseEndpoint
{
     ...
     */
    protected string $url = "https://api.sky.blackbaud.com/alt-conmg/constituents/{constituent_id}/addresses";

    /**
     * This operation returns all address contact inoperationation for a
     * constituent.
     *
     ...
     *   rows.
     *
     * @return \Blackbaud\SKY\Altru\Constituent\Components\ConstituentAddressListCollection
     ...
     */
    public function searchByConstituentId(array $params): ConstituentAddressListCollection
    {
        ...
        return new ConstituentAddressListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['include_former','limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
