...
use Blackbaud\SKY\Altru\Constituent\Components\ConstituentPhoneListCollection;
...
class Phones extends BaseEndpoint
{
     ...
     */
    protected string $url = "https://api.sky.blackbaud.com/alt-conmg/constituents/{constituent_id}/phones";
     ...
     *   rows.
     *
     * @return \Blackbaud\SKY\Altru\Constituent\Components\ConstituentPhoneListCollection
     ...
     */
    public function searchByConstituentId(array $params): ConstituentPhoneListCollection
    {
        ...
        return new ConstituentPhoneListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
