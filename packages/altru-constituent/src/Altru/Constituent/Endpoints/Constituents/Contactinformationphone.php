 ...
 * @api
 */
class Contactinformationphone extends BaseEndpoint
{
    /**
     * @var string $url Endpoint URL pattern
     */
    protected string $url = "https://api.sky.blackbaud.com/alt-conmg/constituents/{constituent_id}/contactinformationphone";

    /**
     * This operation returns all phone contact inoperationation for a
     * constituent.
     *
     ...
     */
    public function searchByConstituentId(array $params): ConstituentContactListCollection
    {
        ...
        return new ConstituentContactListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['include_former','limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
