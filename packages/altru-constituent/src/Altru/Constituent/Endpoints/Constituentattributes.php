...
class Constituentattributes extends BaseEndpoint
{
    ...
    public function getByConstituentAttributeId(array $params): ConstituentAttribute
    {
        ...
        return new ConstituentAttribute($this->send("get", array_filter($params, fn($key) => in_array($key, ['constituent_attribute_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY)));
    }
    ...
    public function deleteByConstituentAttributeId(array $params): mixed
    {
        ...
        return $this->send("delete", array_filter($params, fn($key) => in_array($key, ['constituent_attribute_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY));
    }
    ...
    public function patchOnConstituentAttributeId(array $params, EditConstituentAttribute $requestBody): mixed
    {
        ...
        return $this->send("patch", array_filter($params, fn($key) => in_array($key, ['constituent_attribute_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY), $requestBody);
    }
}
