...
use Blackbaud\SKY\Altru\Constituent\Components\UpdateConstituentAttribute;
...
class Constituentattributes extends BaseEndpoint
{
     ...
     */
    public function postBy(NewConstituentAttribute $requestBody): PostResponse
    {
    ...
    public function getByConstituentAttributeId(array $params): ConstituentAttribute
    {
        ...
        return new ConstituentAttribute($this->send("get", ["{constituent_attribute_id}" => $params['constituent_attribute_id']], []));
    }
     ...
     *   parameters are not defined
     */
    public function deleteByConstituentAttributeId(array $params): void
    {
        assert(isset($params['constituent_attribute_id']), new ArgumentException("Parameter `constituent_attribute_id` is required"));

        return $this->send("delete", ["{constituent_attribute_id}" => $params['constituent_attribute_id']], []);
    }
     ...
     *   array
     *     - constituent_attribute_id: The constituent attribute id.
     * @param \Blackbaud\SKY\Altru\Constituent\Components\UpdateConstituentAttribute
     *   $requestBody ConfigurationMessage object representing operation
     *   intended to be created
     *
     ...
     *   parameters are not defined
     */
    public function patchByConstituentAttributeId(array $params, UpdateConstituentAttribute $requestBody): void
    {
        assert(isset($params['constituent_attribute_id']), new ArgumentException("Parameter `constituent_attribute_id` is required"));
        assert(isset($params['requestBody']), new ArgumentException("Parameter `requestBody` is required"));

        return $this->send("patch", ["{constituent_attribute_id}" => $params['constituent_attribute_id']], [], $requestBody);
    }
}
