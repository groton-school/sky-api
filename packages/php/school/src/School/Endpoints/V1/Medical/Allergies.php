...
class Allergies extends BaseEndpoint
{
    ...
    public function deleteByAllergyId(array $params): int
    {
        ...
        return $this->send("delete", array_filter($params, fn($key) => in_array($key, ['allergy_id']), ARRAY_FILTER_USE_KEY), []);
    }
    ...
    public function patchOnAllergyId(array $params, StudentAllergyUpdate $requestBody): int
    {
        ...
        return $this->send("patch", array_filter($params, fn($key) => in_array($key, ['allergy_id']), ARRAY_FILTER_USE_KEY), [], $requestBody);
    }
