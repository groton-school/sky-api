...
class UserAdd implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets homeLanguages
     *
     * @param \SKY\School\Model\HomeLanguageAdd[]|null $homeLanguages A list of languages the user speaks at home. Each entry is the description or ID of the language. Descriptors are not case sensitive, but otherwise must match the table value exactly. These values are returned by GET Types table values.
     *
     * @return self
     */
    ...
    public function setIsInternational($isInternational)
    {
        if (is_null($isInternational)) {
            array_push($this->openAPINullablesSetToNull, 'isInternational');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('isInternational', $nullablesSetToNull);
            if ($index !== false) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
