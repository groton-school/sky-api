...
class Sessions extends BaseEndpoint
{
    ...
    public function list_(array $params): SchoolSessionCollection
    {
        ...
        return new SchoolSessionCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['level_num','school_year']), ARRAY_FILTER_USE_KEY)));
    }
}
