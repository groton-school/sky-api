...
class Conditions extends BaseEndpoint
{
    ...
    public function deleteByConditionId(array $params): int
    {
        ...
        return $this->send("delete", array_filter($params, fn($key) => in_array($key, ['condition_id']), ARRAY_FILTER_USE_KEY), []);
    }
    ...
    public function patchOnConditionId(array $params, StudentConditionUpdate $requestBody): int
    {
        ...
        return $this->send("patch", array_filter($params, fn($key) => in_array($key, ['condition_id']), ARRAY_FILTER_USE_KEY), [], $requestBody);
    }
