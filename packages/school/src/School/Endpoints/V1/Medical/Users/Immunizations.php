...
class Immunizations extends BaseEndpoint
{
    ...
    public function getByUserId(array $params): array
    {
        ...
        return array_map(fn($a) => new StudentImmunizationRead($a), $this->send("get", array_filter($params, fn($key) => in_array($key, ['user_id']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['include_']), ARRAY_FILTER_USE_KEY)));
    }
}
