...
use Blackbaud\SKY\Altru\Administration\Components\CurrencyListCollection;
...
class List_ extends BaseEndpoint
{
     ...
     */
    protected string $url = "https://api.sky.blackbaud.com/alt-adnmg/currencies/list";

    /**
     * Lists all currencies in the system.
     *
     * @param array{includeinactive: bool, limit: int, session_key: string,
     ...
     *   rows.
     *
     * @return \Blackbaud\SKY\Altru\Administration\Components\CurrencyListCollection
     ...
     */
    public function list_(array $params): CurrencyListCollection
    {
        ...
        return new CurrencyListCollection($this->send("get", array_filter($params, fn($key) => in_array($key, ['']), ARRAY_FILTER_USE_KEY), array_filter($params, fn($key) => in_array($key, ['includeinactive','limit','session_key','infinity_session','more_rows_range_key','start_row_index']), ARRAY_FILTER_USE_KEY)));
    }
}
